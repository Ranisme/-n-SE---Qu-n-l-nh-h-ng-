...
const create = async (payload) => {
  const { ten, khuVucId, posX, posY, width, height, shape } = payload;
...
const remove = async (id) => {
  ...
  const orders = await prisma.donHang.count({
    where: { banId: id, trangThai: 'open' },
  });
  if (orders > 0) {
    throw Object.assign(new Error('Không thể xóa bàn đang có order'), { status: 400 });
  }
  
  await prisma.ban.delete({ where: { id } }).catch(() => null);
  await broadcastTables();
